...
class WSF(SocialRecommender):
    def __init__(self,conf,trainingSet=None,testSet=None,fold='[1]'):
        super(WSF, self).__init__(conf,trainingSet,testSet,fold)
        self.config = conf
    ...
    def getBetweenCentrality(self,G,load=True):
        self.communication = np.zeros(self.dao.trainingSize()[0])
    ...
    def buildModel(self):
        ...
        while iteration < self.maxIter:
            ...
            for triple in self.dao.trainingData:
                ...
                if suv != 0:
                    self.S[u] += self.lRate*(1-self.alpha)*error*((r*suv - trustRating)/(suv**2))
