...
class W2V(IterativeRecommender):
    def __init__(self,conf,trainingSet=None,testSet=None,fold='[1]'):
        super(W2V, self).__init__(conf,trainingSet,testSet,fold)

    def initModel(self):
        super(W2V, self).initModel()
    ...
    def buildModel(self):
        ...
        while iteration < self.maxIter:
            ...
            for entry in self.dao.trainingData:
                ...
                self.H += self.lRate * (error*x.T.dot(y))
            iteration += 1
    ...
    def predict(self,u,i):
        if self.dao.containsUser(u) and self.dao.containsItem(i):
            ...
            u = self.dao.getUserId(u)
            i = self.dao.getItemId(i)
            return self.dao.globalMean + self.Bi[i] + self.Bu[u] + x.dot(self.H).dot(y.T)[0][0]
        else:
